...
export default async function NewDeliveryPage({ searchParams }: PageProps) {
      ...
      return (
        <div className="max-w-2xl space-y-4">
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-800">
            ...
            <Link href="/procurement/deliveries" className="underline hover:text-amber-900">
              Back to Deliveries
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      >
        <Link
          href={po ? `/procurement/purchase-orders/${po_id}` : '/procurement/deliveries'}
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {activePOs.map((p) => (
                <Link
                  key={p.id}
                  href={`/procurement/deliveries/new?po_id=${p.id}`}
